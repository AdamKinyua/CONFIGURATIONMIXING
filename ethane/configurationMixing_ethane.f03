 program coeffsMixing
    ...
    ! Variables ----------------------------------------------------
        ...
        type(mqc_scf_integral)::homo_8,homo_7,homo_6,homo_5,homo_4,homo_3,homo_2,homo_1,homo,lumo,lumo_1,lumo_2 ! mos of interest for ethylene
        type(mqc_scf_integral)::ihomo_8,ihomo_7,ihomo_6,ihomo_5,ihomo_4,ihomo_3,ihomo_2,ihomo_1,ihomo,ilumo,ilumo_1,ilumo_2 ! i means initial
        type(mqc_gaussian_unformatted_matrix_file)::temp_file
        type(mqc_scalar)::normHomo,normHomo_1,normHomo_2,c2c0,c2c1,c1c0
        integer(kind=int64)::mo1=1,mo2=2,mo3=3,mo4=4,mo5=5,mo6=6,mo7=7,mo8=8,mo9=9,mo10=10,mo11=11,mo12
...
! MO1 ----> homo-7 for ethylene
            ihomo_8=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo1],axis=2)
            tmpmatrix = ihomo_8%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial HOMO-8 at '//trim(num2char(i)))

            homo_8=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo1],axis=2)
            tmpmatrix = homo_8%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'HOMO-8 vector at '//trim(num2char(i)))
            call mqc_print(tmpvector%norm('F'),6,'HOMO-8 vector norm at '//trim(num2char(i)))

    ! MO2 ----> homo-7 for ethylene
            ihomo_7=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo2],axis=2)
            tmpmatrix = ihomo_7%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial HOMO-7 at '//trim(num2char(i)))

            homo_7=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo2],axis=2)
            tmpmatrix = homo_7%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'HOMO-7 vector at '//trim(num2char(i)))
            call mqc_print(tmpvector%norm('F'),6,'HOMO-7 vector norm at '//trim(num2char(i)))
    

    ! MO3 ----> homo-6 for ethylene
            ihomo_6=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo3],axis=2)
            tmpmatrix = ihomo_6%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial HOMO-6 at '//trim(num2char(i)))

            homo_6=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo3],axis=2)
    ...
    ! MO4 ----> homo-5 for ethylene
            ihomo_5=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo4],axis=2)
            tmpmatrix = ihomo_5%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial HOMO-5 at '//trim(num2char(i)))

            homo_5=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo4],axis=2)
    ...
    ! MO5 ----> homo-4 for ethylene
            ihomo_4=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo5],axis=2)
            tmpmatrix = ihomo_4%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial HOMO-4 at '//trim(num2char(i)))

            homo_4=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo5],axis=2)
    ...
    ! MO6 ------> homo-3 for ethylene
            ihomo_3=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo6],axis=2)
            tmpmatrix = ihomo_3%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial HOMO-3 at '//trim(num2char(i)))

            homo_3=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo6],axis=2)
    ...
    ! MO7 ------> homo-2 for ethylene
            ihomo_2=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo7],axis=2)
            tmpmatrix = ihomo_2%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial HOMO-2 at '//trim(num2char(i)))

            homo_2=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo7],axis=2)
    ...
    ! MO8 ------> homo-1 for ethylene
            ihomo_1=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo8],axis=2)
            tmpmatrix = ihomo_1%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial HOMO-1 at '//trim(num2char(i)))

            homo_1=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo8],axis=2)
    ...
    ! MO9 -------> homo for ethylene
            ihomo=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo9],axis=2)
            tmpmatrix = ihomo%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial HOMO at '//trim(num2char(i)))

            homo=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo9],axis=2)
    ...
    ! MO10 --------> lumo for ethylene
            ilumo=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo10],axis=2)
            tmpmatrix = ilumo%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial LUMO at '//trim(num2char(i)))

            lumo=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo10],axis=2)
    ...
    ! MO11 --------> lumo+1 for ethylene
            ilumo_1=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo11],axis=2)
            tmpmatrix = ilumo_1%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial LUMO+1 at '//trim(num2char(i)))

            lumo_1=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo11],axis=2)
    ...
    ! MO12 --------> lumo+2 for ethylene
            ilumo_2=mo_listIntegral_i%orbitals('useStrings',betaOrbsIn=[mo12],axis=2)
            tmpmatrix = ilumo_2%getBlock('beta')
            tmpvector = tmpmatrix%vat([0],[1])
            call tmpvector%print(6,'Initial LUMO+2 at '//trim(num2char(i)))

            lumo_2=overlapQuantifier%orbitals('useStrings',betaOrbsIn=[mo12],axis=2)
